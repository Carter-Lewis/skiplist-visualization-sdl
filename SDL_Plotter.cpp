...
void SDL_Plotter::drawCircle(int r, point p, color c) {
    for (int x = -r; x <= r; x++) {
        for (int y = -r; y <= r; y++) {
            if (sqrt(pow(x, 2) + pow(y, 2)) < r) {
                plotPixel(x + p.x, y + p.y, 255, 255, 255);
            }
            if (abs(sqrt(pow(x, 2) + pow(y, 2)) - r) < 1) {
...
void SDL_Plotter::displayNode(int val, int x, int y, color c) {
    drawCircle(30, {x, y}, c);
    plotNumber(val, x, y);
}